       ...
       */
        ...
        * @param   $coll_b	=	array(	'collection'	集合B的名称,key为数据库查询字段，val为查处数据字短
        ...
        public static function updateAll($query = array() , $new_object ,
            ...
            if(!empty($options['safe'])) return $ret['n'];
            else return $ret;
...
//连接mongodb xwarrior @2012/7/20
    function connect_mongodb()
    {
        ...
        BaseMongoRecord::$connection = new Mongo( $GLOBALS['mongodb_config']['host']);
        BaseMongoRecord::$database =   $GLOBALS['mongodb_config']['database'];
        //启用从库读
     //   BaseMongoRecord::$connection->setSlaveOkay(true);
    ...
    if (!BaseMongoRecord::$connection){
        try{
            ...
            try{
                ...
                \Debug::log('Error:connect to mongodb fail', $GLOBALS['mongodb_config']['host']);
                //die("connect to at server fail(已经过2次尝试)");
            }
        }
    }
