 ...
 * 注意： URL_PATH_UPLOAD 在_LOCAL/local.inc.php
 */
class FileUpload{
	/**
	 * 允许传入的文件格式
	...
	public static function create($src_filename, $tmp_file_path_name, $check_image = false){
		...
		if($from_net){
			//模拟浏览器抓取
			$cu = new \tools\MultiProcess(array(CURLOPT_REFERER=>$tmp_file_path_name, CURLOPT_SSL_VERIFYHOST=>false, CURLOPT_SSL_VERIFYPEER=>false));
		...
		if(!file_exists(ROOT_PATH . '/DocumentRoot/upload/' . $file_path_name))return false;
	...
	{
	    if(preg_match('/http:\/\/fan\.yoka\.com\/fanimages\/(.*)/i',$file_url_path,$matches) || preg_match('/http:\/\/fanp[1-4]\.yokacdn\.com\/(.*)/i', $file_url_path,$matches))
	 ...
	 */
	private function mkdirs($pathStr,$mod=0755,$own='www.www'){
	...
	public static function get_net_pic($url, $referer='', $cookie='') {
		...
		list($src, $tmp) = explode('?', basename($tmp_file_path_name));
		$ext = 'jpg';	// 默认设置为.jpg文件扩展
