 ...
 * @modify: xwarrior
**/  
class YLSecuritySecoder {
    //验证码中使用的字符，01IO容易混淆，不用  
    public static $codeSet = '3456789ABCDEFGHJKLMNPRTUVWXY';   
    ...
    public static $bg = array(253, 251, 254);  // 背景   
     ...
     * @code 验证码
     */  
    public static function entry($code_str) {
        ...
        // 验证码字体随机颜色   
        self::$_color = imagecolorallocate(self::$_image, mt_rand(1,90), mt_rand(1,90), mt_rand(1,90));   
        // 验证码使用随机字体，保证目录下有这些字体集   
        $ttf = dirname(__FILE__) . '/ttfs/t' . mt_rand(1, 8) . '.ttf';     
        ...
        // 绘验证码   
        $code = str_split($code_str); // 验证码 
         
        $codeNX = 5; // 验证码第N个字符的左边距   
        foreach ($code as $char) {   
            // 写一个验证码字符   
            imagettftext(self::$_image, self::$fontSize, mt_rand(-30, 30), $codeNX, self::$fontSize*1.4, self::$_color, $ttf,  $char);   
            $codeNX += mt_rand(self::$fontSize*1.15, self::$fontSize*1.3);   
        }   
...
YL_Security_Secoder::$fontSize = 22;
YL_Security_Secoder::$imageH = 36;   //高
YL_Security_Secoder::$imageL = 110;  //长
YL_Security_Secoder::$length = 4;
